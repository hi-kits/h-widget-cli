...
async function dlAction() {
  // 清空模版文件夹的相关内容，用法见 fs-extra 的 README.md
  try {
  ...
  try {
    // 下载模版
    await download(jsonConfig.mirror, path.resolve(__dirname, '../template/'), {extract:true});
  } catch (err) {
    // 下载失败时提示
