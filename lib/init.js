...
// 请求 handlebars 库，用于替换模版字符
const handlebars = require('handlebars');

const path = require('path');

// 请求 download.js 文件，模版不在本地时执行该操作
  ...
  try {
    ...
    if (exists) {
      ...
      inquirer.prompt([
        ...
        // 拼接 template 文件夹路径
        const templatePath = path.resolve(__dirname, '../template/js-plugin-cli-1-master')
        ...
        for (var i = 0; i < multiFiles.length; i++) {
          // 这里使用 try {} catch {}, 以便出错时可以终止掉 Spinner
          try {
            
            // 等待读取文件
            const multiFilesContent = await fse.readFile(multiFiles[i], 'utf8');
            // 等待替换文件, handlebars.compile(源文件内容)(模版字符)
